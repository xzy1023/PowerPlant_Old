...
-- =============================================
-- WO#1297      Cot. 1, 2014   Bong Lee
-- Description:	Post process for downloading MS Dynamics AX pallet adjustments to Power Plant for facility 01
-- =============================================
CREATE PROCEDURE [dbo].[PPsp_PostProcess_DownLoadPalletAdjustment_01]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @chrFacility = Right(OBJECT_NAME(@@PROCID), Charindex('_', Reverse(OBJECT_NAME(@@PROCID)))-1);
		...
		-- Call generic stored procedure to run post process of the pallet adjustment data Down Load for specific facility
		SET @vchSQLStmt =  N'EXEC ' + @vchPPDBName + N'.dbo.PPsp_PostProcess_DownLoadPalletAdjustment @chrFacility';
