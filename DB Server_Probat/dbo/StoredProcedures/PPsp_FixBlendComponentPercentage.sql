...
CREATE PROCEDURE [dbo].[PPsp_FixBlendComponentPercentage]
AS
BEGIN
SET NOCOUNT ON;
    ...
    SELECT @maxFixedCustomerId = Value1 FROM tblControl WHERE [Key] = 'Probat' AND [SubKey] = 'FixBlendComponentPercentage';
	...
	SELECT CUSTOMER_ID FROM dbo.PRO_IMP_BLENDS WHERE CUSTOMER_ID > @maxFixedCustomerId and CUSTOMER_ID <= @maxTableCustomerId;
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		FROM dbo.PRO_IMP_BLENDS WHERE CUSTOMER_ID = @id;
	...
	UPDATE tblControl SET Value1 = @maxTableCustomerId WHERE [Key] = 'Probat' AND [SubKey] = 'FixBlendComponentPercentage';
