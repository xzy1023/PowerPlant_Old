Public Class frmQATCaseDateCoder
    '---------------------------------------------------------------------------------
    'Author:        Bong Lee
    ...
    Private Sub frmQATCaseDateCoder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub dgvPrtDev_CellMouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvPrtDev.CellMouseDown
        ...
        Dim drDynamicLabelData As dsDynamicLabelData.PPsp_DynamicLabelData_SelRow
        ...
        Try
                ...
                If e.RowIndex >= 0 AndAlso dgvPrtDev.Columns(e.ColumnIndex).Name = "dgvPrtDev_btnPrtName" Then
                    With dgvPrtDev.Rows(e.RowIndex)
                        Try
                            ...
                            Using taDynamicLabelData As New dsDynamicLabelDataTableAdapters.PPsp_DynamicLabelData_SelTableAdapter
                                Using dtDynamicLabelData As New dsDynamicLabelData.PPsp_DynamicLabelData_SelDataTable
                                    taDynamicLabelData.Fill(dtDynamicLabelData, Nothing, CASELABELER, gdrSessCtl.DefaultPkgLine, gdrSessCtl.ShopOrder)
                                    If dtDynamicLabelData.Count > 0 Then
                                        drDynamicLabelData = dtDynamicLabelData.Rows(0)
                                        If Not drDynamicLabelData.IsPreFmtProductionDateNull Then
                                            lblPreFmtProductionDate.Text = "Prd: " & drDynamicLabelData.PreFmtProductionDate
                                            lblPreFmtProductionDate.Visible = True
                                            lblPreFmtProductionDateDesc.Visible = True
                                        End If

                                        If Not drDynamicLabelData.IsPreFmtExpiryDateNull Then
                                            lblPreFmtExpiryDate.Text = drDynamicLabelData.PreFmtExpiryDate
                                            lblPreFmtExpiryDate.Visible = True
                                            lblPreFmtExpiryDateDesc.Visible = True
                                        End If
                                    End If
                                End Using
                            End Using
            ...
            Else
                ...
                strMsg = "Network connection is not ready, Please check the IPC connection to the network."
    ...
    Private Sub btnRetest_Click(sender As Object, e As EventArgs) Handles btnRetest.Click
        ...
        Try
            ...
            lblPreFmtProductionDate.Visible = False
            lblPreFmtExpiryDate.Visible = False
            lblPreFmtProductionDateDesc.Visible = False
            lblPreFmtExpiryDateDesc.Visible = False
            ...
            SharedFunctions.printCaseLabel(CASELABELER, String.Empty)
    ...
    Private Sub LoadPrinterList(Optional ByVal strWhen As String = "")
        ...
        Dim strDeviceTypeFilter As String = String.Empty
        ...
        Try
            ...
            If My.Computer.Network.IsAvailable = True Then
                Try
                    ...
                    strDeviceTypeFilter = "DeviceType = '" & CASELABELER & "'"
                    Me.dgvPrtDev.DataSource = dt.Select(strDeviceTypeFilter)
                    dgvPrtDev.Refresh()
                Catch sqEx As SqlClient.SqlException
                    strMsg = "Can not retrieve the printer list for the line. Please check the IPC or data base server connection."
                    SharedFunctions.PoPUpMSG(strMsg, strMsgBoxTitle, MessageBoxButtons.OK)
                End Try
            Else
                If strWhen = "me.Load" Then
                    strMsg = "Please check the IPC network connection."
                Else
                    strMsg = "Please check the IPC network connection before any printer network connections."
                End If
                SharedFunctions.PoPUpMSG(strMsg, strMsgBoxTitle, MessageBoxButtons.OK)
            End If
        Catch ex As Exception
            Throw New Exception("Error in LoadPrinterList" & vbCrLf & ex.Message)
        End Try
    End Sub

    Private Sub btnFail_Click(sender As Object, e As EventArgs) Handles btnFail.Click
        Try
            SaveCaseDateCoderResult(TestResult.Failed)
    ...
    Private Sub btnPass_Click(sender As Object, e As EventArgs) Handles btnPass.Click
        Try
            SaveCaseDateCoderResult(TestResult.Passsed)
    ...
    Private Sub btnNA_Click(sender As Object, e As EventArgs) Handles btnNA.Click
        Try
            SaveCaseDateCoderResult(TestResult.NA)
            SharedFunctions.UpdateQATStatus(False, False, gdrSessCtl.ShopOrder, strCurrQATEntryPoint, drwf.QATDefnID, gstrInterfaceID, drwf.TestSeq)
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message & vbCrLf & ex.StackTrace, "** Unexpected Application Error - Contact Supervisor", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub SaveCaseDateCoderResult(intTestResult As Integer)
        Try
            Dim strDateCodeValue As String = "Unknown"
            Dim dteTestTime As Nullable(Of DateTime) = Now
            Const cintDateCodeType As Integer = 3
            ' Date code type:  where 1 = Package Coder, 2 = Cartonbox Coder, 3 = Case Coder

            If lblPreFmtExpiryDate.Text <> String.Empty Then
                strDateCodeValue = lblPreFmtExpiryDate.Text
            End If

            'If the test is not applicable, do not create a detail test result record by setting the test time to nothing.
            ...
            SharedFunctions.SaveQATDateCodeResult(gdteTestBatchID, strDateCodeValue, cintDateCodeType, gdrSessCtl.Facility, gstrInterfaceID, gdrSessCtl.DefaultPkgLine, intRetestNo,
                                      gdrSessCtl.ShopOrder, gdrSessCtl.StartTime, Now, intTestResult, dteTestStartTime, dteTestTime, gstrQATTesterID, strCurrQATEntryPoint)
            'WO#17432 ADD Stop – AT 11/15/2018
        Catch ex As Exception
            Throw New Exception("Error in SaveCaseDateCoderResult" & vbCrLf & ex.Message)
        End Try
    End Sub
