Public Class frmQATCartonDateCoder
    '---------------------------------------------------------------------------------
    'Author:        Bong Lee
    ...
    Private Sub frmCartonDateCoder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If My.Computer.Network.IsAvailable = True Then
                Try
                    ...
                    strDeviceTypeFilter = "DeviceType = '" & PACKAGECODER & "' AND DeviceSubType = 'B'"
    ...
    Private Sub btnFail_Click(sender As Object, e As EventArgs) Handles btnFail.Click
        Try
            SaveCartonBoxDateCoderResult(TestResult.Failed)
    ...
    Private Sub btnPass_Click(sender As Object, e As EventArgs) Handles btnPass.Click
        Try
            SaveCartonBoxDateCoderResult(TestResult.Passsed)
    ...
    Private Sub btnNA_Click(sender As Object, e As EventArgs) Handles btnNA.Click
        Try
            SaveCartonBoxDateCoderResult(TestResult.NA)
            SharedFunctions.UpdateQATStatus(False, False, gdrSessCtl.ShopOrder, strCurrQATEntryPoint, drwf.QATDefnID, gstrInterfaceID, drwf.TestSeq)
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message & vbCrLf & ex.StackTrace, "** Unexpected Application Error - Contact Supervisor", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub SaveCartonBoxDateCoderResult(intTestResult As Integer)
        Try
            Dim strDateCodeValue As String = "N/A"
            Dim dteTestTime As Nullable(Of DateTime) = Now
            Const cintDateCodeType As Integer = 2
            ' Date code type:  where 1 = Package Coder, 2 = Carton Box Coder, 3 = Case Coder
            ...
            SharedFunctions.SaveQATDateCodeResult(gdteTestBatchID, strDateCodeValue, cintDateCodeType, gdrSessCtl.Facility, gstrInterfaceID, gdrSessCtl.DefaultPkgLine, intRetestNo,
                                gdrSessCtl.ShopOrder, gdrSessCtl.StartTime, Now, intTestResult, dteTestStartTime, dteTestTime, gstrQATTesterID, strCurrQATEntryPoint)        ' WO#17432 AT 10/05/2018
            'WO#17432 ADD Stop – AT 11/15/2018
        Catch ex As Exception
            Throw New Exception("Error in SaveCartonBoxDateCoderResult" & vbCrLf & ex.Message)
        End Try
    End Sub
