Public Class frmQATLineClearance
    ...
    Private Sub frmQATLineClearance_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            If Not IsNothing(drWF) Then             ' WO#17432 – AT 11/29/2018
                ...
                InitializeScreen(gdrSessCtl.Facility, gdrSessCtl.DefaultPkgLine, drWF.QATDefnID, strCurrQATEntryPoint)
    ...
    Private Sub UpdateLineClearance()
        ...
        Try
            ...
            For Each row As DataRow In dtTaskStatus.Rows
                If row.Field(Of Integer)(1) <> 1 Then
                    SharedFunctions.SaveQATLineClearance(gdteTestBatchID, row.Field(Of Integer)(2), gdrSessCtl.Facility, gstrInterfaceID, gdrSessCtl.DefaultPkgLine, _
                        intShopOrder, gdrSessCtl.StartTime, dteJobEndTime, dteJobStartTime, gstrQATTesterID, strCurrQATEntryPoint, _
                        row.Field(Of DateTime)(4), row.Field(Of Integer)(0), row.Field(Of DateTime)(3), row.Field(Of Integer)(1))
                End If
            Next

        Catch ex As Exception
            Throw New Exception("Error in UpdateLineClearance" & vbCrLf & ex.Message)
        End Try
    End Sub
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        ...
        Try
            ...
            UpdateLineClearance()                       'WO#17432 AT 12/21/2018
    ...
    Private Sub btnTask_click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            ' WO#17432 DEL Start – AT 12/21/2018
            '' WO#17432 ADD Start – AT 9/26/2018
            '' Get task start time and disable ByPassAllTest button
            'If btnStatus.Text = cstrNotDone Then
            '    Panel1.Visible = True
            '    txtTaskDesc.Text = sender.text & ":"
            '    txtTaskDesc.Tag = btnCurrent.Tag
            ...
            '    ' WO#17432 ADD Stop – AT 9/26/2018
            'Else
            '    MessageBox.Show("The status of the selected task has been updated, please select another tasks.", "Task Selection", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'End If
            ' WO#17432 DEL Stop – AT 12/21/2018
        Catch ex As Exception
    ...
    Private Sub btnSelectedStatus_Click(sender As Object, e As EventArgs) Handles btnOption1.Click, btnOption2.Click
        ...
        Try
            ...
            'WO#17432 AT 12/21/2018 SaveLineClearance(0, intTaskStatus, intCurrentTaskID)
    ...
    Private Sub InitializeScreen(strFacility As String, strPackagingLine As String, intQATDefnID As Integer, strQATEntryPoint As String)
        Dim btnTasks As Button()
        Dim btnStatuss As Button()
        Dim btnTask As Button
        Dim btnStatus As Button
        Dim btnByPassAll As Button

        Dim intBtnX As Int16, intBtnY As Int16
        Dim intBtnWidth As Int16 = 215, intBtnHeight As Int32 = 63
        Dim intIntialBtnX As Int16 = 27, intIntialBtnY As Int16 = 70
        Dim intBtnXGap As Int16 = 403, intBtnYGap As Int16 = 80

        Dim intBtnStatusX As Int16, intBtnStatusY As Int16
        Dim intBtnStatusWidth As Int16 = 111, intBtnStatusHeight As Int16 = 63
        Dim intIntialBtnStatusX As Int16 = 252, intIntialBtnStatusY As Int16 = 70
        Dim intBtnStatusXGap As Int16 = 403, intBtnStatusYGap As Int16 = 80
            ...
            Using taQATTask As New dsQATTaskTableAdapters.CPPsp_QATTask_SelTableAdapter
                Using dtQATTask As New dsQATTask.CPPsp_QATTask_SelDataTable
                    ...
                    If dtQATTask.Count > 0 Then
                        ...
                        ReDim btnStatuss(intNoOfTasks - 1)
                        ...
                        For intTaskNo = 0 To intNoOfTasks - 1
                            ...
                            With btnTask
                                ...
                                .Text = drQATTask.TaskDescription
                            ...
                            btnStatuss(intTaskNo) = btnStatus
                        ...
                        End If
                    Else
                        MessageBox.Show("Cannot find the QAT line clearance tasks for this line. Please contact QA.", "Error - Missing QAT information", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        CloseForm()
                        Exit Sub
                    End If
                End Using
    ...
    End Sub
    ' WO#17432 ADD Start – AT 9/26/2018
    Private Sub SaveLineClearance(ByVal intByPassAllTest As Integer, ByVal intTaskStatus As Integer, ByVal intTaskID As Integer)
        Dim intShopOrder As Integer
        Try
            If strCurrQATEntryPoint = cstrClosing Then
                intShopOrder = intClosedShopOrder
            Else
                intShopOrder = gdrSessCtl.ShopOrder
            End If
            ' Update Line Clearance header and detail
            SharedFunctions.SaveQATLineClearance(gdteTestBatchID, intByPassAllTest, gdrSessCtl.Facility, gstrInterfaceID, gdrSessCtl.DefaultPkgLine, _
                 intShopOrder, gdrSessCtl.StartTime, dteJobEndTime, dteJobStartTime, gstrQATTesterID, strCurrQATEntryPoint, _
                    ...
                    If Microsoft.VisualBasic.Left(ctl.Name, 8) = "btnTask_" Then
                        ...
                        If strTaskType = cstrNext Then
                            If strTaskStatus = cstrNotDone Then
                                SaveLineClearance(0, intTaskStatus, intTaskID)
                            End If
                        Else
                            If strTaskIDStatus <> "" Then
                                SaveLineClearance(1, intTaskStatus, intTaskID)
                            End If
                        End If
                    End If
                End If
