...
-- INC 50755	Sept 25, 2024
-- Description: Add using PkgLabelDateFmtCode on the X type label for the date format
-- INC xxxx		Nov 21 2025 Sagar K/Zhiyuan X 
-- Description:	Updated code to insert MPExpiryDate
-- =============================================
CREATE PROCEDURE [dbo].[PPsp_CreateLabelData] 
...
--WO#1096	DECLARE @vchPreFmtExpiryDate as varchar(30);
	...
	BEGIN TRY
		...
		SELECT @intShelfLife = ProductionShelfLifeDays, 
		...
		-- INC 50755
		IF @chrLabelType = 'X' and @chrFacility = '09'
			SELECT @chrLblFmtCode = PkgLabelDateFmtCode FROM dbo.tblItemMaster WHERE facility = @chrFacility AND ItemNumber = @vchItemNo;
		ELSE
			SELECT @chrLblFmtCode = LabelDateFmtCode FROM dbo.tblItemMaster WHERE facility = @chrFacility AND ItemNumber = @vchItemNo;
		...
		IF @chrLabelType <> 'P'
			...
			--	ELSE
				...
				INSERT INTO tblDynamicLabelData
					(Facility, RecordType, LabelKey, PackagingLine, ShopOrder, ItemNumber, MPProductionDate, MPExpiryDate,
					 ProductionDate, PreFmtProductionDate, PreFmtExpiryDate, Operator, LotID, DefaultPkgLine)
					VALUES (@chrFacility, @chrLabelType, @vchLabelKey, @chrPkgLine, @intShopOrder, @vchItemNo, dbo.fnConvertDate(@dteProductionDate, '01', @chrFacility, @intShift), dbo.fnConvertDate(DATEADD(day,@intShelfLife,@dteProductionDate), '01', @chrFacility, @intShift),
...
-- Add#0001 begin
				  UPDATE tblDynamicLabelData 
					SET MPProductionDate = dbo.fnConvertDate(@dteProductionDate, '01', @chrFacility, @intShift),
						MPExpiryDate = dbo.fnConvertDate(DATEADD(day,@intShelfLife,@dteProductionDate), '01', @chrFacility, @intShift),
			...
			-- Pallet Label
				...
				INSERT INTO tblDynamicLabelData
					(Facility, RecordType, LabelKey, PackagingLine, ShopOrder, ItemNumber, PalletID, Quantity,
					  MPProductionDate, MPExpiryDate, ProductionDate, PreFmtProductionDate,  Operator, LotID, DefaultPkgLine)
					VALUES (@chrFacility, @chrLabelType, @vchLabelKey, @chrPkgLine, @intShopOrder, @vchItemNo, @intPalletID, @intQuantity,
							dbo.fnConvertDate(@dteProductionDate, '01', @chrFacility, @intShift),dbo.fnConvertDate(DATEADD(day,@intShelfLife,@dteProductionDate), '01', @chrFacility, @intShift),@dteProductionDate, dbo.fnConvertDate(@dteProductionDate, @chrLblFmtCode, @chrFacility, @intShift), @vchOperator, @vchLotID, @chrDefaultPkgLine)	
			END
	END TRY
	BEGIN CATCH
