...
-- V6.77		Oct 10 2023 Sagar Kumthekar/Zhiyuan Xiao
-- Description: Check if the pallet label is printed manually by adding IsManualRequest column
-- ==========================================================================
CREATE PROCEDURE [dbo].[PPsp_CreatePrintRequest]
	...
	@bitIsManualRequest bit = 0							--V6.77
AS
BEGIN
		...
		--WO#512 ADD Start
			IF  @chrLabelType = 'P'
				INSERT INTO tblLabelPrintLog
					(TimeSubmit, facility, LabelType, DefaultPkgLine, StartTime, JobName, DeviceName, SbmFromPalletStation, Requestor, LabelID, IsManualRequest)
				VALUES (@dteTimeSubmit, @chrFacility, @chrLabelType,@chrDftPkgLine, @dteStartTime, @vchJobName, @vchDeviceName, @bitSbmFromPalletStation, @vchRequestor,
					LTRIM(Substring(@vchJobName, LEN(@chrDftPkgLine) + 1, LEN(@vchJobName)- LEN(@chrDftPkgLine) + 1)), @bitIsManualRequest)
		COMMIT TRANSACTION
		--WO#512 ADD Stop
